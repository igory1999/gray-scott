...
void print_settings(const Settings &s)
{
    ...
    std::cout << "pd:     " << s.pd << std::endl;
    std::cout << "d:     " << int(s.L*s.pd) << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    timing("Starting gray-scott simulation", log);
	...
	{
	  std::ostringstream message;
	  message << "gray-scott:computation:start " << i;
	  timing(message.str(), log);
	}
#endif
	...
	{
	  std::ostringstream message;
	  message << "gray-scott:computation:end " << (i - settings.plotgap);
	  timing(message.str(), log);
	}
        timer_write.start();
	{
	  std::ostringstream message;
	  message << "gray-scott:write:start " << (i - settings.plotgap);
	  timing(message.str(), log);
	}
#endif
	...
	{
	  std::ostringstream message;
	  message << "gray-scott:write:end " << (i - settings.plotgap);
	  timing(message.str(), log);
	}
